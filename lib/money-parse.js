...
//
// The alphabetic alternatives are anchored with a leading \b so they cannot
// match inside an ordinary word — case-insensitive "rs" is otherwise a
// substring of "hours", "users", "towers" and any other word ending in
// those two letters, followed by a number. A trailing \b is deliberately
// *not* added: "Rs. 1,694.07" has "Rs." followed by a space, and \b requires
// a word character on both sides of the boundary, so a trailing anchor after
// the optional "." would fail to match real input. The leading \b alone is
// enough — in "hours" the "rs" is preceded by "u", a word character, so the
// boundary already fails there. Symbol alternatives (€ £ ₹ $) are left
// unanchored: they are not word characters, and \b behaves counter-intuitively
// around them.
const CURRENCY = "\\bUS\\$|\\bEUR|\\bGBP|\\bINR|\\bUSD|\\bCHF|\\bCZK|\\bTHB|\\bRs\\.?|\\bK[čc]|€|£|₹|\\$";
