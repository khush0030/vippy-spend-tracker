...
import { linkReceipt, setAsideRestricted, billItems } from "./match-service.js";
import { restrictedGoods } from "./restricted.js";
import { listMailAccounts, getMailAccount } from "./mail-account.js";
import { openMailbox } from "./mailbox.js";
import { searchWindow, candidateFrom, merchantFrom } from "./harvest-plan.js";
...
async function storeDocument({ userId, account, message, link, line }) {
  ...
  // Alcohol or tobacco on it: never filed, and its charge needs no bill. An
  // attached invoice is read for its items; the email's own text is checked
  // either way.
  const hits = await restrictedInDocument({ userId, message, rendered });
  if (hits.length) {
    await setAsideRestricted({ userId, receiptId, hits, transactionId: line.transaction_id || null });
    return receiptId;
  }
...
async function restrictedInDocument({ userId, message, rendered }) {
  const text = [message.subject, message.text || String(message.html || "").replace(/<[^>]+>/g, " ")].filter(Boolean).join("\n");
  let items = [];
  if (rendered.kind === "attachment") {
    try {
      const { extractReceipt } = await import("./receipt-vision.js");
      const read = await extractReceipt(rendered.buffer, "application/pdf", { userId });
      if (read.ok) items = billItems(read.raw);
    } catch (err) {
      await logWarn({ source: "harvest", event: "item_read_failed", userId, message: `Could not read items: ${String(err.message).slice(0, 160)}` });
    }
  }
  return restrictedGoods({ text, items });
}
