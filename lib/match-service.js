...
import { scoreCandidate, decide, expectedInrBand, findSplit, isSameBill, isCovered } from "@/lib/matcher";
...
export async function linkReceipt({ receiptId, transactionId, userId, score, matchedBy = "auto" }) {
  ...
  if (existing?.length) {
    const [{ data: txn }, { data: bills }] = await Promise.all([
      sb.from("transactions").select("amount").eq("id", transactionId).eq("user_id", userId).maybeSingle(),
      sb.from("receipts").select("amount, currency").in("id", existing.map((l) => l.receipt_id)),
    ]);
