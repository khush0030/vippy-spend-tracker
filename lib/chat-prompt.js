...
export function systemPrompt({ today, cycle, billing = cycle, cardLabel }) {
  ...
  return [
    ...
    "Two different things are 'pending': charges without a receipt (missing_receipts) and receipts the user sent that have not matched a charge yet (waiting_receipts). For a vague question like 'what is pending' or 'what is left', check both. When a waiting receipt has could_be, offer to file it with match_receipt. When missing_receipts lists sent_but_not_filed, say those bills are already here and just need filing, and propose them with match_receipt.",
...
export function receiptNote(receipt, verdict) {
  ...
  else if (verdict.action === "restricted") outcome = `Not filed — it has ${verdict.hits.join(", ")} on it, and bills with alcohol or tobacco never go to accounts.${verdict.best ? ` Transaction ${verdict.best.transaction_id} is marked as needing no bill.` : ""}`;
