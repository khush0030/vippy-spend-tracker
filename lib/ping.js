...
export async function runPing(userId) {
  ...
  for (const txn of chosen) {
    ...
    // Reserve the row first: a ping that reached Telegram but whose row failed
    // to write would be sent again next tick. If the reservation is refused
    // (another tick got there first) the charge is skipped; if the send fails
    // the reservation is released so it is tried again.
    const { error: reserveErr } = await sb.from("receipt_pings").insert({ transaction_id: txn.id, user_id: userId });
    if (reserveErr) continue;
    try {
      const msg = await sendMessage(chatId, text, { keyboard });
      await sb.from("receipt_pings").update({ tg_message_id: msg?.message_id ?? null }).eq("transaction_id", txn.id);
      sent++;
    } catch (err) {
      await sb.from("receipt_pings").delete().eq("transaction_id", txn.id).is("answered_at", null);
