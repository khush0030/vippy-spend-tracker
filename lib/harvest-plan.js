...
/**
 * Senders whose mail quotes charges without ever being a bill for them. The
 * card issuer's alerts and OTPs state the exact amount of every purchase, so
 * left in they would "cover" every statement line with a non-document.
 */
const NOT_A_MERCHANT = /hdfcbank/i;

export function isBankAlert(message) {
  return NOT_A_MERCHANT.test(String(message?.from || ""));
}

export function candidateFrom(message) {
  if (!message?.date) return null;
  if (isBankAlert(message)) return null;
