...
- corners: where the receipt sits in the photograph — [{x,y},{x,y},{x,y},{x,y}] as fractions of the image width and height, ordered top-left, top-right, bottom-right, bottom-left. Null for a PDF or a screenshot that is already just the document.
...
8. For corners, mark the edges of the PAPER, not the print and not the table. Follow the paper even where it is creased or in shadow, and where it runs past the edge of the photograph clamp that corner to the edge. An approximate answer is genuinely fine — the crop is widened and then trimmed by measurement — but a corner placed inside the printed area is not.`;
