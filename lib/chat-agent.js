...
export async function chatTurn({ userId, chatId, text }) {
  ...
  if (out.confirm) out.fresh[out.fresh.length - 1] = { role: "assistant", content: `Proposed: ${out.confirm.summary}. Waiting for the user to tap Confirm.` };
  ...
  if (out.confirm) {
    ...
    const combined = `<b>${esc(out.confirm.summary)}?</b>\n\nNothing changes until you tap Confirm.`;
    const keyboard = [[
