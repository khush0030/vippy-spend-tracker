...
// `plain: true` sends the text as-is, with no HTML parsing.
export async function sendMessage(chatId, text, { keyboard, replyTo, silent, plain } = {}) {
...
/**
 * Send an image so it appears inline in the chat.
 *
 * Documents arrive as a file to tap; a photo is visible at a glance, which is
 * the point when the question is "did that crop go wrong".
 */
export async function sendPhoto(chatId, buffer, filename, { caption, keyboard } = {}) {
  const form = new FormData();
  form.append("chat_id", String(chatId));
  if (caption) {
    form.append("caption", caption);
