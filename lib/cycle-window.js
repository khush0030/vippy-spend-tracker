...
/** The cycle that closed most recently before the one containing `refDate`. */
export function previousWindow(statementDay, refDate = new Date()) {
  const { start } = cycleWindow(statementDay, refDate);
  const dayBefore = new Date(`${start}T00:00:00Z`);
  dayBefore.setUTCDate(dayBefore.getUTCDate() - 1);
  return cycleWindow(statementDay, dayBefore);
}

/**
 * The card is Indian and so are its cut-offs; the server runs on UTC. At half
 * past midnight in Mumbai on the 17th it is still the 16th in UTC, and the
 * statement has already closed.
 */
export function localToday(now = new Date(), timeZone = "Asia/Kolkata") {
  return new Intl.DateTimeFormat("en-CA", { timeZone, year: "numeric", month: "2-digit", day: "2-digit" }).format(now);
}

/**
 * The cycle whose receipts are being chased on `today`.
 *
 * Once a statement closes, its cycle stays the one on screen until the package
 * for it is due at accounts (or has been sent) — the open cycle is a few days
 * old and its bill is a month away. After that, the open cycle.
 */
export function billingWindow({ statementDay, submitDay, today, previousSubmitted = false }) {
  const open = cycleWindow(statementDay, today);
  const prev = previousWindow(statementDay, today);
  const due = cycleMilestones(prev.end, submitDay, today);
  if (!previousSubmitted && due && today <= due.submitDate) return { ...prev, closed: true };
