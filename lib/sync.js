...
import { pairRowsWithEmails, canonicalMerchant } from "@/lib/sync-pairing";
import { fetchHDFCEmails } from "@/lib/gmail";
import { getSupabaseAdmin, getSupabase } from "@/lib/supabase";
import { logError, logInfo, logWarn } from "@/lib/logger";

const SYNC_MODEL = process.env.SYNC_MODEL || "sarvam:sarvam-105b";
const SYNC_MODEL_FALLBACK = process.env.SYNC_MODEL_FALLBACK || "openai:gpt-5.4-mini";
...
4. MERCHANT NAMING — canonicalize consistently:
   ...
   - Use the brand people know, never the legal entity: "Zomato Limited" → "Zomato", "Blink Commerce Pvt Ltd" → "Blinkit", "RAMEN-ISM B.V." → "Ramen-ism". Drop Limited, Ltd, Pvt, Private, Inc, B.V., GmbH, s.r.o.
   - Card descriptors often prefix a terminal or operator code ("LS ", "MSW ", "SumUp *", "RAZ*"): drop it

5. Categories: amazon, fuel, dining, swiggy, utilities, subscriptions, office, travel, other.

Return ONLY a valid JSON array, one object per email you did not skip. Each object must have: emailId (copied exactly from the "(id: ...)" line of the email it came from), merchant, amount, date, category, itemDescription, isRefund, notes, time.`;
...
export async function parseBatch(emailBatch) {
  ...
  // Both providers failed: throw, so the caller's retry and its
  // "do not advance the cursor" guard see this batch as failed, and the
  // emails are read again tomorrow rather than skipped forever.
  if (!text) throw lastErr || new Error("no model produced a batch");

  const jsonMatch = stripJsonFence(text).match(/\[[\s\S]*\]/);
  // A legitimately empty batch is the literal "[]", which this regex still
  // matches — anything that doesn't match is an unparseable or truncated
  // answer, and must fail rather than be read as "nothing to insert".
  if (!jsonMatch) throw new Error("model answer held no JSON array");

  const parsed = JSON.parse(jsonMatch[0]);
  return pairRowsWithEmails(parsed, emailBatch).map(([t, email]) => {
    // Use the model's extracted time, or fall back to regex extraction from email body/header
    const time = t.time || extractTimeFromEmail(email.body, email.date);
    return {
      email_id: email.id,
      merchant: canonicalMerchant(t.merchant),
 ...
 */
export async function syncUserTransactions({ userId, refreshToken = null }) {
  ...
  let emails;
  try {
    emails = await fetchHDFCEmails(lastSyncedAt, { refreshToken });
  } catch (err) {
    await logError({
      source: "sync",
  ...
  try {
    ...
    for (const row of aliasRows || []) {
      ...
      // Rows are parsed with canonicalMerchant() already applied, so an
      // alias recorded against the pre-canonicalisation string ("Blink
      // Commerce Pvt Ltd") would otherwise never match a new row's merchant.
      aliasMap.set(canonicalMerchant(row.original_merchant), row.alias);
    }
  } catch {}
  const applyAlias = (r) => {
  ...
  // A card hold and its release arrive as a charge alert and a refund alert.
  // Neither is ever billed, so neither needs a bill: both are waived rather
  // than deleted, and a statement that does bill them still ties them out.
  if (totalInserted > 0) {
    try {
      const { findReversals } = await import("@/lib/reversals");
      const admin = getSupabaseAdmin();
      const since = new Date(Date.now() - 14 * 86400e3).toISOString().slice(0, 10);
      const { data: recent, error } = await admin
        .from("transactions")
        .select("id, merchant, amount, date, is_refund, receipt_status")
        .eq("user_id", userId)
        .gte("date", since);
      if (error) throw new Error(error.message);
      const pairs = findReversals(recent || []);
      if (pairs.length) {
        const { data: waived, error: waiveError } = await admin
          .from("transactions")
          .update({ receipt_status: "waived", declared_reason: "charged and reversed the same day" })
          .in("id", pairs.flat())
          .eq("user_id", userId)
          .eq("receipt_status", "missing")
          .select("id");
        if (waiveError) throw new Error(waiveError.message);
        if (waived?.length !== pairs.length * 2) throw new Error(`waived ${waived?.length ?? 0} of ${pairs.length * 2} rows`);
        await logInfo({ source: "sync", event: "reversals", userId, message: `Waived ${pairs.length} reversed hold(s)`, details: { pairs } });
      }
    } catch (err) {
      await logWarn({ source: "sync", event: "reversals_failed", userId, message: "Could not settle reversed holds", details: { error: err.message } });
    }
  }
