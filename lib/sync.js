...
async function parseBatch(emailBatch) {
  ...
  const response = await anthropic.messages.create({
    // Pinned to a family, not a dated snapshot: the previous pin was retired
    // and every batch 404'd for two months while the cron reported "0 new".
    model: process.env.SYNC_MODEL || "claude-sonnet-5",
    max_tokens: 4096,
    messages: [{ role: "user", content: `${SYSTEM_PROMPT}\n\nEmails:\n${emailSummaries}` }],
  });

  // Newer models may lead with a non-text block, so take every text block
  // rather than assuming the first one is the answer.
