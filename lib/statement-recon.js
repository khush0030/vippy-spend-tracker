...
import { reconcile, findAbsorbed } from "@/lib/recon";
...
export async function reconcileStatement({ userId, statementId, asOf = null }) {
  ...
  // --- 3b. alerts that are one of these lines in another form ---

  await absorbDuplicates({ userId, statementId: statement.id }).catch((err) =>
    logWarn({ source: "statement", event: "absorb_failed", userId, message: String(err.message).slice(0, 200) })
  );
 ...
 */
export async function runStatementJob({ userId, force = false, asOf = null, refreshToken = null } = {}) {
  const ingest = await ingestStatements({ userId, force, refreshToken });
...
/**
 * Fold charges that duplicate one of this statement's lines into that line's
 * charge (see findAbsorbed): bills move across, the duplicate is deleted.
 * With `apply: false` it only reports. Returns the absorptions.
 */
export async function absorbDuplicates({ userId, statementId, apply = true }) {
  const sb = getSupabaseAdmin();
  const { data: statement } = await sb.from("statements").select("id, period_start, period_end").eq("id", statementId).eq("user_id", userId).maybeSingle();
  if (!statement) throw new Error(`Statement ${statementId} not found`);

  const { data: lines } = await sb
    .from("statement_lines")
    .select("txn_date, descriptor, amount, currency, amount_orig, transaction_id, recon_status, type")
    .eq("statement_id", statementId)
    .eq("type", "purchase");
  const onLines = new Set((lines || []).map((l) => l.transaction_id).filter(Boolean));

  const { data: leftovers } = await sb
    .from("transactions")
    .select("id, merchant, amount, date, is_refund")
    .eq("user_id", userId)
    .is("statement_line_id", null)
    .gte("date", shift(statement.period_start, -7))
    .lte("date", statement.period_end);

  const found = findAbsorbed(
    (lines || []).map((l) => ({ ...l, created: l.recon_status === "created" })),
    (leftovers || []).filter((t) => !onLines.has(t.id))
  );
  if (!apply) return found;

  for (const a of found) {
    const { data: links } = await sb.from("receipt_transactions").select("*").eq("transaction_id", a.from);
    for (const l of links || []) {
      const { error } = await sb.from("receipt_transactions").upsert({ ...l, transaction_id: a.into }, { onConflict: "receipt_id,transaction_id" });
      if (error) throw new Error(`moving bill to ${a.into}: ${error.message}`);
    }
    if (links?.length) {
      await sb.from("transactions").update({ receipt_status: "attached", has_receipt: true, declared_reason: null }).eq("id", a.into).eq("user_id", userId);
    }
    await sb.from("receipt_transactions").delete().eq("transaction_id", a.from);
    const { error } = await sb.from("transactions").delete().eq("id", a.from).eq("user_id", userId);
    if (error) throw new Error(`deleting duplicate ${a.from}: ${error.message}`);
    await logInfo({ source: "statement", event: "absorbed", userId, message: `Charge ${a.from} folded into ${a.into}: ${a.why}`, details: a });
  }
  return found;
}
