...
async function parseBatch(emailBatch) {
  // Trim email bodies to reduce token usage
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (err?.status === 429 || err?.error?.type === "rate_limit_error") {
        const waitTime = 65000 * attempt; // 65s, 130s, 195s
...
export async function parseEmailsToTransactions(emails) {
  ...
  // 5 emails per batch + 65s delay to stay well within 10k tokens/min
  const BATCH_SIZE = 5;
  const DELAY_BETWEEN_BATCHES = 65000; // 65s — just over 1 min to reset rate limit
  ...
  for (let i = 0; i < emails.length; i += BATCH_SIZE) {
    ...
    console.log(`Progress: ${allTransactions.length} transactions parsed so far`);

    // Wait between batches to avoid rate limits
    if (i + BATCH_SIZE < emails.length) {
      console.log(`Waiting 65s before next batch (${batchNum}/${totalBatches} done)...`);
