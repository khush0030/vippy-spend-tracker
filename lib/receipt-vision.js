...
export async function extractReceipt(buffer, mime, { userId } = {}) {
  let readerA, readerB;
  try {
    readerA = readerFor(MODEL_A);
    readerB = readerFor(MODEL_B);
  } catch (err) {
    // A bad VISION_MODEL_* env (e.g. "openai" with no ":model") throws here,
    // before either provider is ever called — report it as a failed read
  ...
  let tie;
  try {
    tie = readerFor(MODEL_TIEBREAK, { reasoningEffort: "high" });
  } catch (err) {
    return {
      ok: true,
      consensus: "conflict",
