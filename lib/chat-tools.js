...
export const TOOL_DEFS = [
  ...
  tool("missing_receipts", "Charges still without a receipt in a period (default this cycle). Also returns receipts_waiting: receipts the user already sent that have not matched a charge (count and newest few; waiting_receipts lists all). count is the number of charges missing.", {
    period: periodParam(),
  }, []),
  tool("waiting_receipts", "Receipts the user sent (photo or PDF) that are saved but not matched to a charge yet — still being read, or waiting for the bank alert. Newest first.", {}, []),
...
async function waitingReceipts(sb, userId) {
  const { data, error } = await sb.from("receipts")
    .select("merchant, amount, currency, receipt_date, status, created_at")
    .eq("user_id", userId)
...
const READ = {
  ...
  async missing_receipts({ sb, userId, ctx, args }) {
    ...
    return {
      ...
      receipts_waiting: { count: waiting.length, newest: waiting.slice(0, 5) },
  ...
  async waiting_receipts({ sb, userId }) {
    ...
    return { count: rows.length, ...trim(rows) };
  },
