...
import { findCandidates, linkReceipt, billsAwaitingFiling, openMissingCharges, suggestForReceipt, discardReceipt } from "@/lib/match-service";
...
export const TOOL_DEFS = [
  ...
  tool("waiting_receipts", "Receipts the user sent (photo or PDF) that are saved but not matched to a charge yet — still being read, or waiting for the bank alert. Newest first. could_be is the closest charge (transaction id, score out of 100) when one is plausible; offer it with match_receipt. Without could_be, maybe_misread lists charges with the same merchant or exact amount — the bill was probably misread (wrong amount or date); ask which one it is, or offer discard_receipt if it is not a work bill.", {}, []),
  ...
  tool("discard_receipt", "Throw out a waiting receipt that is not for the card — personal, from another trip, or junk — so it never matches or goes to accounts (asks for confirmation). Only when the user says so.", {
    receipt_ref: { type: "string", description: "8-character receipt_ref from waiting_receipts" },
  }, ["receipt_ref"]),
...
const READ = {
  ...
  async missing_receipts({ sb, userId, ctx, args }) {
    // "Missing receipts" is about the bill being paid, however the question is
    // worded: this cycle, last cycle, the statement, or no period at all. Only
    // an explicit other period (a month, a date range) looks elsewhere.
    const asked = resolvePeriod(args.period, ctx);
    const b = ctx.billing;
    const meansBill = b && (!args.period || ["billing_cycle", "this_cycle", "last_cycle"].includes(args.period) || (asked.start === b.cycle_start && asked.end === b.cycle_end));
    const p = meansBill ? resolvePeriod("billing_cycle", ctx) : asked;
    // The billing cycle is its statement's lines once that is reconciled, not a date range.
    const billed = meansBill ? await cycleScope(b) : null;
    const all = billed
    ...
    const toFile = await billsAwaitingFiling(userId, { onlyCharges: new Set(rows.map((t) => t.id)) });
    return {
      // Bills the user already sent for charges in this list, waiting on a Confirm: offer each with match_receipt.
      sent_but_not_filed: toFile.map((f) => ({ receipt_ref: f.receipt.id.slice(0, 8), bill: `${f.receipt.merchant} ${f.receipt.currency} ${f.receipt.amount} (${f.receipt.receipt_date})`, transaction_id: f.txn.id, charge: `${f.txn.merchant} ₹${f.txn.amount} (${f.txn.date})` })),
  ...
  async waiting_receipts({ sb, userId }) {
    ...
    const open = await openMissingCharges(userId).catch(() => []);
    await Promise.all(shown.map(async (r) => {
      ...
      else if (r.full.status === "unmatched") {
        const loose = await suggestForReceipt(userId, r.full, { charges: open, limit: 3 });
        if (loose.length) r.maybe_misread = loose.map(({ txn }) => ({ transaction_id: txn.id, merchant: txn.merchant, amount: txn.amount, date: txn.date }));
      }
    }));
...
const PROPOSE = {
  ...
  async discard_receipt({ sb, userId, args }) {
    const { data: open } = await sb.from("receipts").select("id, merchant, amount, currency, receipt_date").eq("user_id", userId).in("status", ["pending", "unmatched"]).limit(500);
    const r = (open || []).find((x) => x.id.startsWith(args.receipt_ref));
    if (!r) return { error: `no waiting receipt with receipt_ref ${args.receipt_ref} — check waiting_receipts` };
    return { confirm: { kind: "discard_receipt", summary: `Discard ${r.merchant || "receipt"} ${r.currency || "INR"} ${r.amount} (${r.receipt_date}) — not a work bill`, payload: { receipt_id: r.id } } };
  },
...
export async function executeWrite({ userId, kind, payload }) {
  ...
  switch (kind) {
    ...
    case "discard_receipt":
      if (!(await discardReceipt({ userId, receiptId: payload.receipt_id }))) throw new Error("that receipt is already filed or set aside");
      return "Discarded — it won't match or go to accounts.";
    case "match_receipt": {
