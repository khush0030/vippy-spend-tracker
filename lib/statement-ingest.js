 ...
 */
export async function ingestStatements({ userId, since = null, force = false, refreshToken = null } = {}) {
  const card = await getCardAccount(userId);
  if (!card) return { skipped: "no card configured", statements: [] };

  const today = new Date().toISOString().slice(0, 10);
  const emails = await fetchStatementPdfs({ since: since || shift(today, -LOOKBACK_DAYS), refreshToken });
...
async function ingestOne({ userId, card, email }) {
  ...
  // Named stages: a failure here used to surface as a bare TypeError from a
  // minified bundle, with no way to tell decryption from transcription.
  ...
  const password = await stage("password", async () => decryptSecret(card.statement_password));
  const pdf = await stage("decrypt", () => decryptPdf(email.buffer, password));
  const parsed = await stage("transcribe", () => parseStatement(pdf, { userId }));
