...
function isIso(s) {
  // Round-trip: Date quietly rolls 2026-02-30 over to 2 March.
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) return false;
  const d = new Date(`${s}T00:00:00Z`);
  return !Number.isNaN(d.getTime()) && d.toISOString().slice(0, 10) === s;
}

export function resolvePeriod(spec, { today, cycle, billing = cycle }) {
  ...
  if (s === "billing_cycle" && billing) {
    return { start: billing.cycle_start, end: billing.cycle_end, label: `the billing cycle (${dayLabel(billing.cycle_start)} – ${dayLabel(billing.cycle_end)})` };
  }
  ...
  if (isIso(s)) return { start: s, end: s, label: dayLabel(s) };
