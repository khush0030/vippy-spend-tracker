...
export async function loadQpdf({ onOut, onErr } = {}) {
  ...
  return withStringArgv(() =>
    createModule({
      ...
      // than print/printErr, so output arrives one character at a time — both
      // streams, which is how `--show-npages` has to be read.
      stdout: onOut || (() => {}),
      stderr: onErr || (() => {}),
