...
import { pingedTodayUnanswered } from "@/lib/ping";
...
export async function dailyNudge(userId) {
  ...
  if (!due.length) return { sent: false, reason: "nothing outstanding" };

  const lines = due
    .slice(0, 6)
    .map((t) => `<code>${t.date} · ${esc(t.merchant).slice(0, 22)} · ${INR(t.amount)}</code>`);

  const more = due.length > 6 ? `\n<i>+${due.length - 6} more</i>` : "";

  await sendMessage(
    chatId,
    `📌 <b>${due.length} recent charge${due.length === 1 ? "" : "s"} without a receipt</b>\n${lines.join(
      "\n"
    )}${more}\n\nSend a photo and I'll file it.`,
    {
      keyboard: [
        [
          { text: "🚫 No bill exists", callback_data: `dq:${due[0].id}` },
  ...
  await logInfo({
    ...
    event: "daily",
    userId,
    message: `Chased ${due.length} charge(s)`,
  });
