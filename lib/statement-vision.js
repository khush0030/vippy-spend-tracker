...
async function readOnce(pdf, note) {
  // A year-end statement runs past 200 rows, and a truncated transcription is
  // invalid JSON rather than a short answer — so the ceiling is generous and
  // the call streams. The SDK refuses a non-streaming request whose max_tokens
  // implies more than ten minutes of generation, and a real statement takes
  // ~27k output tokens, which is over that line.
