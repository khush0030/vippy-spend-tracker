...
// The first 8 hex digits of the receipt's uuid: short enough that a model copies it right.
const receiptRef = (v, name) => {
  const s = String(v ?? "").trim().toLowerCase().slice(0, 8);
  if (!/^[0-9a-f]{8}$/.test(s)) throw new Error(`${name} must be a receipt_ref from waiting_receipts`);
  return s;
};
const ids = (v, name) => {
  const list = (Array.isArray(v) ? v : v == null ? [] : [v]).map((x) => id(x, name));
  if (!list.length || list.length > 5) throw new Error(`${name} needs 1-5 transaction ids`);
  return [...new Set(list)];
};
...
const SCHEMAS = {
  ...
  waiting_receipts: () => ({}),
  missing_receipts: (a) => ({ period: text(a.period, "period", { required: false, max: 40 }) || "billing_cycle" }),
  receipt_status: (a) => ({
  ...
  match_receipt: (a) => ({ receipt_ref: receiptRef(a.receipt_ref ?? a.receipt_id, "receipt_ref"), transaction_ids: ids(a.transaction_ids ?? a.transaction_id, "transaction_ids") }),
  discard_receipt: (a) => ({ receipt_ref: receiptRef(a.receipt_ref ?? a.receipt_id, "receipt_ref") }),
};

export const TOOL_NAMES = Object.keys(SCHEMAS);
export const WRITE_TOOLS = new Set(["declare_no_bill", "mark_recurring", "rename_merchant", "set_category", "snooze_pings", "match_receipt", "discard_receipt"]);
